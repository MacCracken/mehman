# mehman — Claude Code Instructions
...
**mehman** — sovereign compat / "swallow"-stage surface backend for the AGNOS compositor (Persian مهمان, *guest*)
...
mehman owns the **compat / "swallow"-stage surface backend** for [aethersafha](https://github.com/MacCracken/aethersafha) — hosting the surfaces of *foreign* apps (ones that don't speak the AGNOS-native protocol) as guests inside a [kavach](https://github.com/MacCracken/kavach) sandbox, and presenting them to the compositor as native surfaces. It is XWayland's *real* job done the sovereign way: run a foreign-ABI binary sandboxed, capture its surface, hand it over — **not** an X.Org port, **not** X11→Wayland translation. The platform backend (pixels/input/seat against the kernel) is [`bhumi`](https://github.com/MacCracken/bhumi)'s job, not mehman's. mehman is **post-MVP** (the swallow stage); bhumi ships first.
...
cyrius build programs/smoke.cyr build/mehman-smoke
